...
W celu umożliwienia kontrolowania działania \texttt{Raspberry~Pi}, zainstalowano serwer WWW \texttt{tornado}, na którym skonfigurowano interfejs użytkownika z autoryzacją przez prosty formularz \texttt{HTML}. Konfiguracja interfejsu www jest w pliku \path{/root/server.py} oraz w katalogu \path{/root/static}.
\begin{figure}[H]
\centering
\begin{tabular}{r|l}
Login: & \texttt{admin}\\
...
Do zapewnienia regularnej synchronizacji czasu przez protokół \texttt{NTP} oraz w celu regularnego \emph{uploadu} regularnie synchronizowanych zdjęć przez \texttt{rsync}, wykorzystano \emph{demona} systemowego \texttt{cron}. Konfiguracja uruchamiania \texttt{cron'a} została zachowana w pliku \path{/etc/crontab} i sprowadza się do uruchamiania co minutę skryptu \path{/root/sync.sh} o zawartości przedstawionej na listingu~\ref{lst:sync}.
